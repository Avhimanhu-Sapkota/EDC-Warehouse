    ...
    <body>
        <?php
            ...
            while($row=mysqli_fetch_array($qry)){
                echo"<div class='empty-box'></div><h3 style='text-align:center;text-transform:uppercase;'>".$row['CATEGORY_NAME']." </h3> <hr style='width:5%;border:1.5px solid black'>";
                echo"<div class='empty-box' style='height:35px'></div> <div class='container'><div class='row'>";

                include ('connection.php');
            $categoryID=$_GET['cid'];
            $sql="SELECT * FROM product WHERE fk1_CATEGORY_ID=$categoryID";
            $qry=mysqli_query($db, $sql);
                    
            while($row=mysqli_fetch_array($qry)){

                if ($row['fk1_CATEGORY_ID'] == $categoryID){ 
